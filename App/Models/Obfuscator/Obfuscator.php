...
namespace App\Models\Obfuscator;
...
class Obfuscator
{
    ...
    public function encode_form()
    {
        return base64_encode(
            mcrypt_encrypt(MCRYPT_BLOWFISH, $this->secret_key, serialize($this->data), MCRYPT_MODE_ECB, $this->iv)
        );
    }
    ...
    public function decode_form($data, $form)
    {
        $retrun_array = array();
        $fields = unserialize(
            mcrypt_decrypt(MCRYPT_BLOWFISH, $this->secret_key, base64_decode($data), MCRYPT_MODE_ECB, $this->iv)
        );
